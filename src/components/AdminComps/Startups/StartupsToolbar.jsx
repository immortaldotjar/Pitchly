...
import { industryFilters, statusFilters } from "../../../config/adminStartupsConfig"

const StartupsToolbar = ({ search, onSearchChange, activeIndustry, onIndustryChange, status, onStatusChange }) => {
    return (
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-6 pb-4">
            ...
            <div className="flex flex-wrap gap-2">
                {industryFilters.map((industry) => (
                    <button
                        ...
                        className={`px-3.5 py-2 rounded-lg font-body-sm text-[12px] font-semibold transition-colors duration-150 ${activeIndustry === industry
            ...
            <div className="w-full sm:w-44 sm:ml-auto">
                <Select id="status-filter" value={status} onChange={(e) => onStatusChange(e.target.value)} options={statusFilters} />
            </div>
        </div>
    )
}

export default StartupsToolbar